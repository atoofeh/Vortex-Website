...
import { ProofBlocks } from "@/components/proof-blocks";
    ...
    <main id="main-content">
      ...
      <section id="architecture" className="section-wrap py-24 sm:py-36">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-gold/25 bg-[#2D0812]/55 p-5 sm:p-8"><div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(212,175,55,0.16),transparent_45%)]" /><div className="relative space-y-2">{["infrastructure", "intelligence", "application", "experience"].map((key, index) => <div key={key} className="relative"><div className={`flex items-center gap-4 rounded-2xl border p-5 ${index === 0 ? "border-gold/40 bg-gold/10" : "border-gold/15 bg-[#1F050C]/60"}`}><span className="font-mono text-[0.62rem] text-gold">0{index + 1}</span><span className="text-sm font-bold uppercase tracking-[0.14em] text-cream sm:text-base">{t(`architecture.${key}`)}</span></div>{index < 3 && <div className="flex h-8 items-center justify-center text-gold/60"><ArrowDown size={15} /></div>}</div>)}</div></div>
        </div>
      </section>
