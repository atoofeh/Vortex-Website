...
import { ArrowDown, ArrowRight, BrainCircuit, Check, CloudCog, Code2, Cpu, Database, Globe2, Layers3, LockKeyhole, Network, Server, Smartphone, Sparkles, Workflow, Zap } from "lucide-react";
...
const capabilities = [
  { key: "ai", icon: BrainCircuit, number: "01", href: "/services/artificial-intelligence", service: "AI System Development" },
  { key: "web", icon: Globe2, number: "02", href: "/services/web-development", service: "Website Development" },
  { key: "mobile", icon: Smartphone, number: "03", href: "/services/mobile-development", service: "Mobile Application" },
  { key: "enterprise", icon: Layers3, number: "04", href: "/services/enterprise-software", service: "Enterprise Platform" },
  { key: "infrastructure", icon: Server, number: "05", href: "/services/infrastructure", service: "Private Infrastructure" },
] as const;

const pillars = [
  { key: "digital", icon: Globe2 },
  { key: "mobile", icon: Smartphone },
  { key: "enterprise", icon: Layers3 },
  { key: "intelligent", icon: BrainCircuit },
] as const;
  ...
  return (
    <main id="main-content">
      <section className="relative isolate flex min-h-[100svh] items-center overflow-hidden pb-20 pt-32 sm:pt-40">
        ...
        <div aria-hidden="true" className="absolute end-[-12rem] top-1/3 h-[30rem] w-[30rem] rounded-full bg-burgundy/60 blur-[90px]" />
      ...
      <section id="architecture" className="section-wrap py-24 sm:py-36">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div><p className="eyebrow mb-4"><LockKeyhole size={13} className="text-gold" />{t("architecture.eyebrow")}</p><h2 className="display max-w-xl text-4xl leading-[0.95] text-cream sm:text-6xl">{t("architecture.title")}</h2><p className="mt-6 max-w-lg text-base leading-relaxed text-muted">{t("architecture.description")}</p><p className="mt-8 font-mono text-xs uppercase tracking-widest text-champagne">{t("architecture.note")}</p></div>
          <div className="relative overflow-hidden rounded-[2rem] border border-gold/25 bg-[#2D0812]/55 p-5 sm:p-8"><div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(212,175,55,0.16),transparent_45%)]" />
            <div className="relative space-y-2">
              {["infrastructure", "intelligence", "application", "experience"].map((key, index) => <div key={key} className="relative"><div className={`flex items-center gap-4 rounded-2xl border p-5 ${index === 0 ? "border-gold/40 bg-gold/10" : "border-gold/15 bg-[#1F050C]/60"}`}><span className="font-mono text-[0.62rem] text-gold">0{index + 1}</span><span className="text-sm font-bold uppercase tracking-[0.14em] text-cream sm:text-base">{t(`architecture.${key}`)}</span></div>{index < 3 && <div className="flex h-8 items-center justify-center text-gold/60"><ArrowDown size={15} /></div>}</div>)}
            </div>
          </div>
        </div>
      </section>

      <section id="build" className="relative overflow-hidden py-24 sm:py-36"><div className="section-wrap"><div className="mb-14 max-w-3xl"><p className="eyebrow mb-4"><Zap size={13} className="text-gold" />{t("build.eyebrow")}</p><h2 className="display text-4xl leading-[0.95] text-cream sm:text-6xl">{t("build.title")}</h2><p className="mt-6 text-base leading-relaxed text-muted sm:text-lg">{t("build.subtitle")}</p></div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {capabilities.map(({ key, icon: Icon, number, href, service }, index) => { const item = copy.build[key]; return <motion.article key={key} initial={reducedMotion ? false : { opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.18 }} transition={{ delay: index * 0.05 }} className="group relative flex min-h-[27rem] flex-col overflow-hidden rounded-[1.7rem] border border-gold/20 bg-[#2D0812]/55 p-6 transition-all hover:-translate-y-1 hover:border-gold/45 hover:shadow-[0_20px_45px_rgba(15,2,6,0.45)] xl:min-h-[31rem]"><div aria-hidden="true" className="absolute -end-12 -top-12 h-32 w-32 rounded-full border border-gold/20 opacity-50 transition-transform duration-500 group-hover:scale-150" /><div className="relative flex items-center justify-between"><div className="grid h-11 w-11 place-items-center rounded-xl border border-gold/30 bg-gold/10 text-gold"><Icon size={21} /></div><span className="font-mono text-[0.65rem] text-muted">{number} / 05</span></div><h3 className="relative mt-8 font-display text-2xl font-bold leading-tight text-cream">{item.title}</h3><p className="relative mt-3 text-sm leading-relaxed text-muted">{item.description}</p><ul className="relative mt-6 space-y-2 border-t border-gold/15 pt-5">{item.points.slice(0, 4).map((point) => <li key={point} className="flex gap-2 text-xs leading-relaxed text-cream/85"><Check size={13} className="mt-0.5 shrink-0 text-gold" />{point}</li>)}</ul><div className="relative mt-auto flex items-center justify-between gap-3 pt-7"><Link href={href} className="focus-ring inline-flex items-center gap-2 font-mono text-[0.62rem] font-bold uppercase tracking-wider text-champagne hover:text-cream">{item.cta}<ArrowRight size={13} /></Link><button type="button" onClick={() => onOpenModal(service)} aria-label={`${item.cta} — ${item.title}`} className="focus-ring grid h-8 w-8 shrink-0 place-items-center rounded-full border border-gold/25 text-champagne hover:border-gold hover:bg-gold/10"><Sparkles size={13} /></button></div></motion.article>; })}
        </div>
      </div></section>

      <section className="section-wrap py-24 sm:py-36"><div className="rounded-[2rem] border border-gold/25 bg-[#2D0812]/45 p-7 sm:p-12"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end"><div><p className="eyebrow mb-4"><Sparkles size={13} className="text-gold" />{t("beyond.eyebrow")}</p><h2 className="display text-4xl leading-[0.95] text-cream sm:text-6xl">{t("beyond.title")}</h2></div><div><p className="text-3xl font-medium leading-tight text-champagne sm:text-5xl">{t("beyond.statement")}</p><p className="mt-6 max-w-2xl text-sm leading-relaxed text-muted sm:text-base">{t("beyond.description")}</p></div></div><div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-gold/15 bg-gold/15 md:grid-cols-2">{pillars.map(({ key, icon: Icon }) => <div key={key} className="bg-[#1F050C]/75 p-6 sm:p-7"><Icon size={19} className="text-gold" /><h3 className="mt-5 text-lg font-semibold text-cream">{t(`beyond.${key}.title`)}</h3><p className="mt-2 text-sm leading-relaxed text-muted">{t(`beyond.${key}.description`)}</p></div>)}</div></div></section>
      ...
      <section className="section-wrap py-24 sm:py-32"><div className="grid gap-10 rounded-[2rem] border border-gold/20 bg-gold/5 p-7 sm:p-12 lg:grid-cols-[0.75fr_1.25fr] lg:items-center"><div><p className="eyebrow mb-4"><Check size={13} className="text-gold" />{t("trust.eyebrow")}</p><h2 className="display text-4xl leading-[0.95] text-cream sm:text-5xl">{t("trust.title")}</h2><p className="mt-6 text-sm leading-relaxed text-muted">{t("trust.description")}</p></div><div className="grid gap-3 sm:grid-cols-2">{copy.trust.items.map((item) => <div key={item} className="flex items-start gap-3 border-t border-gold/20 py-4 text-sm font-semibold text-cream"><Check size={15} className="mt-0.5 shrink-0 text-gold" />{item}</div>)}</div></div></section>
...
export function MarketingFooter() {
  const { t } = useLanguage();
  return <footer className="section-wrap flex flex-col gap-7 border-t border-gold/15 py-10 text-xs text-muted sm:flex-row sm:items-end sm:justify-between"><div className="flex items-center gap-3.5"><div className="grid h-10 w-10 place-items-center rounded-full border border-gold/30 bg-gold/10 text-gold"><Sparkles size={18} /></div><div><p className="text-sm font-bold tracking-tight text-cream">VORTEX AI & IT SOLUTIONS</p><p className="mt-1 text-muted/70">{t("footer.location")}</p></div></div><nav aria-label="Footer navigation" className="flex flex-wrap gap-x-5 gap-y-3"><a href="#build" className="hover:text-champagne">{t("footer.capabilities")}</a><a href="#architecture" className="hover:text-champagne">{t("footer.architecture")}</a><a href="#about" className="hover:text-champagne">{t("footer.about")}</a><a href="mailto:contact@vortexmind.co" className="hover:text-champagne">{t("footer.email")}</a></nav></footer>;
}
