...
export function MarketingFooter() {
  const { t } = useLanguage();
  return <footer className="section-wrap flex flex-col gap-7 border-t border-gold/15 py-10 text-xs text-muted sm:flex-row sm:items-end sm:justify-between"><div className="flex items-center gap-3.5"><div aria-hidden="true" className="grid h-10 w-10 place-items-center rounded-full border border-gold/30 bg-gold/10 text-gold"><Sparkles size={18} /></div><div><p className="text-sm font-bold tracking-tight text-cream">VORTEX</p><p className="mt-1 text-muted/70">{t("footer.location")}</p></div></div><nav aria-label="Footer navigation" className="flex flex-wrap gap-x-5 gap-y-3"><Link href="/#build" className="focus-ring rounded-sm hover:text-champagne">{t("footer.capabilities")}</Link><Link href="/#architecture" className="focus-ring rounded-sm hover:text-champagne">{t("footer.architecture")}</Link><Link href="/#about" className="focus-ring rounded-sm hover:text-champagne">{t("footer.about")}</Link><a href="mailto:contact@vortexmind.co" className="focus-ring rounded-sm hover:text-champagne">{t("footer.email")}</a></nav></footer>;
}
