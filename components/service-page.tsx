...
export function ServicePage({ slug }: { slug: ServiceSlug }) {
  ...
  return <main className="min-h-screen pt-32 sm:pt-44"><section className="section-wrap pb-24 sm:pb-36"><Link href="/#build" className="focus-ring inline-flex items-center gap-2 font-mono text-xs uppercase tracking-wider text-muted hover:text-champagne"><ArrowLeft size={14} />{t("hero.secondary")}</Link><div className="mt-10 max-w-5xl"><p className="eyebrow mb-5"><Sparkles size={13} className="text-gold" />{config.eyebrow}</p><h1 className="display text-[clamp(3rem,8vw,7rem)] leading-[0.88] text-cream">{item.title}</h1><p className="mt-8 max-w-3xl text-lg leading-relaxed text-muted sm:text-xl">{item.description}</p><Link href={`/?service=${encodeURIComponent(config.service)}#contact`} className="focus-ring mt-9 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-gold via-champagne to-gold px-7 py-3.5 text-xs font-bold uppercase tracking-wider text-ink shadow-[0_0_25px_rgba(212,175,55,0.35)]">{item.cta}<ArrowRight size={14} /></Link></div></section>
