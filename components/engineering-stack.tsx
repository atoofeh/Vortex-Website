    ...
    <section id="architecture" className="section-wrap py-24 sm:py-36" aria-labelledby="engineering-stack-title">
      <div className="grid gap-14 lg:grid-cols-[0.82fr_1.18fr] lg:items-start lg:gap-16">
        <motion.div initial={reducedMotion ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} className="lg:pt-8">
          ...
          <div className="mt-10 flex items-center gap-3 border-t border-gold/15 pt-5 font-mono text-[0.68rem] font-bold uppercase tracking-[0.14em] text-champagne"><span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-gold shadow-[0_0_12px_rgba(212,175,55,0.8)]" /> Five layers / one connected system</div>
        </motion.div>
        ...
        <motion.div initial={reducedMotion ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: 0.06 }} className="vortex-stack-console lg:mt-2">
          <div className="vortex-stack-console__topline flex items-center justify-between font-mono text-[0.68rem] uppercase tracking-[0.14em] text-champagne/75"><span>VORTEX / Engineering model</span><span className="text-gold">Connected system</span></div>
          <div className="vortex-stack-console__header mt-5 border-b border-gold/20 pb-5"><p className="font-mono text-[0.68rem] uppercase tracking-[0.16em] text-gold">Architecture / 01—05</p></div>

          <div className="vortex-stack-path relative mt-8" role="tablist" aria-label="VORTEX engineering layers">
          ...
          <motion.div key={active.key} initial={reducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} id="engineering-layer-panel" role="tabpanel" tabIndex={0} aria-labelledby={`engineering-layer-tab-${active.key}`} className="vortex-stack-console__body relative mt-10 grid gap-12 border-t border-gold/15 pt-10 md:grid-cols-[0.92fr_1.08fr] md:gap-14">
            ...
            <div className="vortex-stack-active relative">
              ...
              <div className="relative z-[1]">
                <p className="font-mono text-[0.68rem] uppercase tracking-[0.16em] text-gold">Active layer / 0{activeLayer + 1}</p>
                <h3 className="mt-5 font-display text-3xl font-bold leading-none text-cream sm:text-5xl">{active.label}</h3>
                <p className="mt-4 max-w-md text-base leading-relaxed text-muted">{active.subtitle}</p>
                <Link href={active.href} className="focus-ring vortex-stack-cta mt-9 inline-flex items-center gap-3 text-sm font-semibold text-champagne hover:text-cream">Explore {active.label.toLowerCase()} engineering <ArrowRight aria-hidden="true" size={16} /></Link>
              </div>
            </div>
            <div className="relative">
              <p className="mb-4 font-mono text-[0.68rem] uppercase tracking-[0.16em] text-gold">Layer responsibilities</p>
              <ul className="vortex-stack-responsibilities grid gap-x-8 gap-y-1 sm:grid-cols-2">{active.items.map((item, index) => <li key={item} className="group"><span className="vortex-stack-responsibilities__index">0{index + 1}</span><strong>{item}</strong><ArrowRight aria-hidden="true" size={14} className="ms-auto shrink-0 text-gold/55 transition-transform group-hover:translate-x-1 group-hover:text-gold" /></li>)}</ul>
            </div>
          </motion.div>
        </motion.div>
      </div>
