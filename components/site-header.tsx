...
const headerCopy = {
  en: { ai: "AI & Intelligence", software: "Software Engineering", digital: "Digital Experiences", automation: "Automation & Infrastructure", engineering: "Engineering", home: "VORTEX home", openMenu: "Open menu", closeMenu: "Close menu" },
  ar: { ai: "الذكاء والذكاء الاصطناعي", software: "هندسة البرمجيات", digital: "التجارب الرقمية", automation: "الأتمتة والبنية التحتية", engineering: "الهندسة", home: "الرئيسية VORTEX", openMenu: "فتح القائمة", closeMenu: "إغلاق القائمة" },
} as const;
    ...
    <>
      <header className="fixed inset-x-0 top-3.5 z-50 px-4 sm:top-5">
        <div className={`mx-auto grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-full border transition-all duration-150 ${chromeClass}`}>
          <Link href="/" aria-label={header.home} className="vortex-brand focus-ring group flex items-center gap-1.5 rounded-full pr-2"><Image src="/logo-emblem.webp" alt="" width={36} height={36} className={(scrolled ? "h-8 w-8" : "h-9 w-9") + " object-contain transition-transform duration-300 group-hover:scale-105"} priority /><span className="font-display text-sm font-bold tracking-tight text-cream">ORTEX</span></Link>
          ...
          <nav aria-label={t("navigation.menu")} className="site-header-nav hidden items-center gap-1 md:flex">
            <div className="relative" onMouseEnter={() => setCapabilitiesOpen(true)} onMouseLeave={() => setCapabilitiesOpen(false)}>
              ...
              <AnimatePresence>
                {capabilitiesOpen && <motion.div role="menu" initial={reducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="absolute start-0 top-full mt-3 grid w-[38rem] grid-cols-2 gap-3 rounded-2xl border border-gold/25 bg-[#1F050C]/95 p-4 shadow-[0_20px_60px_rgba(15,2,6,0.65)] backdrop-blur-xl">{capabilityGroups.map((group) => <div key={group.labelKey} className="rounded-xl border border-gold/10 bg-[#2D0812]/25 p-2"><p className="px-3 pb-2 pt-1 font-mono text-[0.58rem] font-bold uppercase tracking-[0.14em] text-gold">{header[group.labelKey]}</p>{group.links.map(([labelKey, href, descriptionKey]) => <Link key={href} role="menuitem" href={href} className="focus-ring group block rounded-xl p-3 hover:bg-gold/10"><span className="block text-xs font-semibold text-cream group-hover:text-champagne">{t(`${labelKey}.title`)}</span><span className="mt-1 block text-[0.67rem] leading-relaxed text-muted">{t(descriptionKey)}</span></Link>)}</div>)}</motion.div>}
              </AnimatePresence>
            </div>
            <Link href="/engineering" className="link-underline focus-ring rounded-full px-3 py-1.5 font-mono text-[0.66rem] uppercase tracking-[0.12em] text-muted hover:text-champagne">{header.engineering}</Link>
