...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!solution) return {};
  return { title: `${solution.title} | VORTEX`, description: solution.description, alternates: { canonical: `/ar/solutions/${slug}`, languages: { en: `/solutions/${slug}`, ar: `/ar/solutions/${slug}`, "x-default": `/solutions/${slug}` } } };
}
...
export default async function ArabicSolutionRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><SolutionPage solution={solution} initialLocale="ar" /><div className="page-shell"><MarketingFooter /></div></>;
}
