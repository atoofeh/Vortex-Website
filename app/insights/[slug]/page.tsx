...
export default async function InsightRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const insight = insightBySlug[slug];
  if (!insight) notFound();
  const jsonLd = { "@context": "https://schema.org", "@type": "TechArticle", "headline": insight.title, "description": insight.description, "datePublished": insight.published, "dateModified": insight.published, "mainEntityOfPage": `https://www.vortexmind.co/insights/${insight.slug}`, "author": { "@id": "https://www.vortexmind.co/#organization" }, "publisher": { "@id": "https://www.vortexmind.co/#organization" }, "about": { "@type": "Thing", "name": insight.solutionLabel } };
